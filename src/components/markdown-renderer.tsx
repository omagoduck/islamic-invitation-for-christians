...
import CodeBlockContainer from './codeblock-renderer';
...
const components: Options['components'] = {
  // Paragraph component with consistent spacing
  ...
  ol: ({ node, children, className, ...props }) => (
    <ol className={cn('ml-6 mb-4 list-outside list-decimal space-y-1', className)} {...props}>
      {children}
    </ol>
  ),
  ...
  ul: ({ node, children, className, ...props }) => (
    <ul className={cn('ml-6 mb-4 list-outside list-disc space-y-1', className)} {...props}>
      {children}
    </ul>
  ),
  ...
  h1: ({ node, children, className, ...props }) => (
    <h1
      className={cn('mt-6 mb-3 font-semibold text-3xl leading-tight', className)}
      {...props}
    >
      {children}
    </h1>
  ...
  h2: ({ node, children, className, ...props }) => (
    <h2
      className={cn('mt-5 mb-3 font-semibold text-2xl leading-tight', className)}
      {...props}
    >
      {children}
    </h2>
  ...
  h3: ({ node, children, className, ...props }) => (
    <h3 className={cn('mt-4 mb-2 font-semibold text-xl leading-tight', className)} {...props}>
      {children}
    </h3>
  ),
  h4: ({ node, children, className, ...props }) => (
    <h4 className={cn('mt-4 mb-2 font-semibold text-lg leading-tight', className)} {...props}>
      {children}
    </h4>
  ),
  h5: ({ node, children, className, ...props }) => (
    <h5
      className={cn('mt-3 mb-2 font-semibold text-base leading-tight', className)}
      {...props}
    >
      {children}
    </h5>
  ...
  h6: ({ node, children, className, ...props }) => (
    <h6 className={cn('mt-3 mb-2 font-semibold text-sm leading-tight', className)} {...props}>
      {children}
    </h6>
  ),
  ...
  pre: ({ node, className, children }) => {
    ...
    const codeElement = Children.toArray(children).find(
      (child) =>
        isValidElement(child) && (child.props as any).className?.startsWith('language-')
    );

    if (
      isValidElement(codeElement) &&
      (codeElement.props as any).className
    ) {
      language = (codeElement.props as any).className.replace('language-', '');
    }
